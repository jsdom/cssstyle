...
const prepareBorderShorthands = (properties) => {
  ...
  if (positionTopItems.size === 3) {
    ...
    if (property && item) {
      ...
      if (properties.has("border-image")) {
        const { value: imageValue } = properties.get("border-image");
        if (imageValue === "none") {
          const { value: itemValue } = item;
  ...
  } else if (properties.has("border-image")) {
    const { value: imageValue } = properties.get("border-image");
    if (imageValue === "none") {
...
exports.prepareBorderProperties = (property, value, properties = new Map(), opt = {}) => {
    ...
    for (const [property, item] of borderProperties) {
      if (exports.shorthandProperties.has(property)) {
        ...
        if (property === "border") {
          ...
          if (value) {
            longhandProperties.set("border-image", {
...
exports.normalizeProperties = (properties) => {
