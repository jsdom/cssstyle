...
/**
 * Ensures consistent object shape (Hidden Class) for V8 optimization.
 *
...
const prepareBorderStringValue = ({
  ...
  const { lines, positions, name: nameProperty } = borderElements;
...
const prepareBorderProperties = (property, value, priority, properties, opt = {}) => {
  ...
  let prop2, prop3;
  if (property.length > name.length) {
    // Check if next char is '-'
    if (property.charCodeAt(name.length) !== 45) {
      return;
    }
    // property is like "border-..."
    const remainder = property.substring(name.length + 1);
  ...
  // prop1 is assumed to be 'name' since we checked startsWith(name)
  if (
    (positions.includes(prop2) && prop3 && !lines.includes(prop3)) ||
    (lines.includes(prop2) && prop3)
  ) {
    return;
  }
  const parts = {
...
const prepareBorderShorthands = (properties) => {
  for (const collection of Object.values(borderCollections)) {
    collection.items.clear();
    collection.priorItems.clear();
  }
  for (const [property, item] of properties) {
    ...
    if (linePart && borderCollections[linePart]) {
      ...
      if (priority) {
        collection.priorItems.set(property, { property, value, priority });
      } else {
  ...
  for (const [key, collection] of Object.entries(borderCollections)) {
    const { shorthand, generator, items, priorItems } = collection;
    const requiredSize = [WIDTH, STYLE, COLOR].includes(key) ? 4 : 3;
    if (items.size === requiredSize) {
      const [property, item] = generator(items, shorthand) ?? [];
      if (property && item) {
        properties.set(property, item);
        // For position shorthands (top/right/bottom/left), check border-image compatibility
    ...
    } else if (priorItems.size === requiredSize) {
      const [property, item] = generator(priorItems, shorthand, "important") ?? [];
      if (property && item) {
        properties.set(property, item);
        // For position shorthands
        if ([TOP, RIGHT, BOTTOM, LEFT].includes(key)) {
          if (properties.has(BORDER_IMAGE)) {
            const { value: imageValue } = properties.get(BORDER_IMAGE);
            if (imageValue === NONE) {
              namePriorItems.push(item.value);
            }
          }
        }
      }
