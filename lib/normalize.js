...
const prepareBorderStringValue = ({
  ...
  const nameItem = getPropertyItem(BORDER, properties);
  ...
  if (prop3) {
    ...
    borderItems.set(BORDER, nameItem);
...
const prepareBorderProperties = (property, value, priority, properties, opt = {}) => {
  ...
  const borderImageItem =
    borderItems.get(BORDER_IMAGE) ?? getPropertyItem(BORDER_IMAGE, properties);
...
/**
 * Updates longhand properties for all 4 border positions (top, right, bottom, left).
 *
 * @param {Map} properties - The map of properties to update.
 * @param {string} namePart - The property name prefix (e.g., "border").
 * @param {string} linePart - The property line suffix (e.g., "width", "style", "color").
 * @param {string|Array} value - The value to set (string or array for position-based values).
 * @param {string} priority - The priority of the property.
 */
const updatePositionLonghands = (properties, namePart, linePart, value, priority) => {
  for (const position of borderPositions) {
    ...
    if (shorthandProperties.has(property)) {
      ...
      if (property === BORDER) {
        ...
        for (const lineProperty of Object.keys(lineItems)) {
          ...
          updatePositionLonghands(longhandProperties, namePart, linePart, lineValue, priority);
        }
        if (value) {
        ...
        if (Array.isArray(parsedItem)) {
          ...
          updatePositionLonghands(longhandProperties, namePart, linePart, parsedItem, priority);
        } else if (parsedItem) {
