  ...
  for (const val of values) {
    ...
    if (Array.isArray(value) && value.length === 1) {
      ...
      switch (type) {
        ...
        case "Identifier": {
          if (parsers.isValidPropertyValue("border-width", name, globalObject)) {
            if (parsedValues.has("border-width")) {
              return;
            }
            parsedValues.set("border-width", name);
            break;
          } else if (parsers.isValidPropertyValue("border-style", name, globalObject)) {
            if (parsedValues.has("border-style")) {
              return;
            }
            parsedValues.set("border-style", name);
            break;
          } else if (parsers.isValidPropertyValue("border-color", name, globalObject)) {
            if (parsedValues.has("border-color")) {
              return;
            }
