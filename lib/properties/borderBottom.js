  ...
  for (const val of values) {
    ...
    if (Array.isArray(value) && value.length === 1) {
      ...
      switch (type) {
        ...
        case "Identifier": {
          if (parsers.isValidPropertyValue("border-bottom-width", name, globalObject)) {
            if (parsedValues.has("border-bottom-width")) {
              return;
            }
            parsedValues.set("border-bottom-width", name);
            break;
          } else if (parsers.isValidPropertyValue("border-bottom-style", name, globalObject)) {
            if (parsedValues.has("border-bottom-style")) {
              return;
            }
            parsedValues.set("border-bottom-style", name);
            break;
          } else if (parsers.isValidPropertyValue("border-bottom-color", name, globalObject)) {
            if (parsedValues.has("border-bottom-color")) {
              return;
            }
