...
module.exports.parse = function parse(v, opt = {}) {
  ...
  for (const val of values) {
    const value = parsers.parsePropertyValue("border-bottom", val, {
      globalObject,
      inArray: true
    });
  ...
  set(v) {
    ...
    if (parsers.hasVarFunc(v)) {
      ...
      if (obj === "") {
        ...
        this._setProperty("border", "");
        this._setProperty("border-bottom", [...Object.values(obj)].join(" "));
      }
    }
  },
  get() {
    const val = this.getPropertyValue("border-bottom");
    if (parsers.isGlobalKeyword(val) || parsers.hasVarFunc(val)) {
      return val;
    }
    const subVal = this._shorthandGetter("border-bottom", module.exports.shorthandFor);
