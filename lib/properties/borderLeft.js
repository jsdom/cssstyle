...
module.exports.parse = function parse(v, opt = {}) {
  ...
  for (const val of values) {
    ...
    if (Array.isArray(value) && value.length === 1) {
      ...
      switch (type) {
        case AST_TYPES.CALC: {
          if (isNumber || parsedValues.has(BORDER_LEFT_WIDTH)) {
            return;
          }
          parsedValues.set(BORDER_LEFT_WIDTH, `${name}(${itemValue})`);
          break;
        }
        case AST_TYPES.DIMENSION:
        case AST_TYPES.NUMBER: {
          if (parsedValues.has(BORDER_LEFT_WIDTH)) {
            return;
          }
          const parseOpt = {
            min: 0
          ...
          parsedValues.set(BORDER_LEFT_WIDTH, parsedValue);
          break;
        }
        case AST_TYPES.FUNCTION: {
          if (parsedValues.has(BORDER_LEFT_COLOR)) {
            return;
          }
          ...
          parsedValues.set(BORDER_LEFT_COLOR, parsedValue);
          break;
        }
        ...
        case AST_TYPES.IDENTIFIER: {
          if (parsers.isValidPropertyValue(BORDER_LEFT_WIDTH, name, globalObject)) {
            if (parsedValues.has(BORDER_LEFT_WIDTH)) {
              return;
            }
            parsedValues.set(BORDER_LEFT_WIDTH, name);
            break;
          } else if (parsers.isValidPropertyValue(BORDER_LEFT_STYLE, name, globalObject)) {
            if (parsedValues.has(BORDER_LEFT_STYLE)) {
              return;
            }
            parsedValues.set(BORDER_LEFT_STYLE, name);
            break;
          } else if (parsers.isValidPropertyValue(BORDER_LEFT_COLOR, name, globalObject)) {
            if (parsedValues.has(BORDER_LEFT_COLOR)) {
              return;
            }
            parsedValues.set(BORDER_LEFT_COLOR, name);
            break;
          }
          return;
        }
    ...
    for (const key of keys) {
      if (parsedValues.has(key)) {
        ...
        if (parsedValue !== module.exports.initialValues.get(key)) {
          ...
          if (obj[BORDER_LEFT_WIDTH] && obj[BORDER_LEFT_WIDTH] === "medium") {
            delete obj[BORDER_LEFT_WIDTH];
          }
        }
      }
    }
