  ...
  for (const val of values) {
    ...
    if (Array.isArray(value) && value.length === 1) {
      ...
      switch (type) {
        ...
        case "Identifier": {
          if (parsers.isValidPropertyValue("border-left-width", name, globalObject)) {
            if (parsedValues.has("border-left-width")) {
              return;
            }
            parsedValues.set("border-left-width", name);
            break;
          } else if (parsers.isValidPropertyValue("border-left-style", name, globalObject)) {
            if (parsedValues.has("border-left-style")) {
              return;
            }
            parsedValues.set("border-left-style", name);
            break;
          } else if (parsers.isValidPropertyValue("border-left-color", name, globalObject)) {
            if (parsedValues.has("border-left-color")) {
              return;
            }
