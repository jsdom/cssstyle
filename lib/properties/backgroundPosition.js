...
module.exports.parse = function parse(v, opt = {}) {
  ...
  for (const val of values) {
    const value = parsers.parsePropertyValue("background-position", val, {
      globalObject,
      inArray: true
    });
    if (Array.isArray(value) && value.length) {
      ...
      switch (value.length) {
        case 1: {
          const [part1] = value;
          const val1 = part1.type === "Identifier" ? part1.name : parsers.parseMeasurement([part1]);
          if (val1) {
            if (val1 === "center") {
        ...
        case 2: {
          ...
          const val1 = part1.type === "Identifier" ? part1.name : parsers.parseMeasurement([part1]);
          const val2 = part2.type === "Identifier" ? part2.name : parsers.parseMeasurement([part2]);
          if (val1 && val2) {
        ...
        case 3: {
          ...
          const val2 = part2.type === "Identifier" ? part2.name : parsers.parseMeasurement([part2]);
          const val3 = part3.type === "Identifier" ? part3.name : parsers.parseMeasurement([part3]);
          if (val1 && val2 && val3) {
            let posX = "";
            let offX = "";
            let posY = "";
