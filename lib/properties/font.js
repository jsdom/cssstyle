...
module.exports.parse = function parse(v, opt = {}) {
  ...
  if (!parsers.isValidPropertyValue("font", v)) {
    return;
  }
  ...
  if (fontBlockB) {
          ...
          switch (property) {
            case "font-size": {
              ...
              if (typeof parsedValue === "string") {
                font[property] = parsedValue;
              }
              break;
            }
            case "font-style":
            case "font-weight": {
              if (font[property] === "normal") {
                const longhand = module.exports.shorthandFor.get(property);
                const parsedValue = longhand.parse(part, {
                  globalObject
                });
                if (typeof parsedValue === "string") {
                  font[property] = parsedValue;
    ...
    if (revParts.length === 1) {
      const [part] = revParts;
      const value = parsers.parsePropertyValue("font", part, {
        globalObject,
        inArray: true
      });
