...
/**
 * Parses a <dimension> value, e.g. <frequency>, <time> and <resolusion>.
 *
 * @param {Array<object>} val - The AST value.
 * @param {object} [opt={}] - The options for parsing.
 * @returns {string|undefined} The parsed dimension.
 */
function parseDimension(val, opt = {}) {
...
function resolveNumericValue(value, opt = {}) {
  ...
  switch (itemType) {
    ...
    case AST_TYPES.DIMENSION: {
      ...
      return parseDimension(value, opt);
    }
    case AST_TYPES.GLOBAL_KEYWORD: {
      if (length > 1) {
        return;
