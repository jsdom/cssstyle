...
var stringRegEx = /^("[^"]*"|'[^']*')$/;
var colorRegEx1 = /^#[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]([0-9a-fA-F])?([0-9a-fA-F][0-9a-fA-F][0-9a-fA-F])?([0-9a-fA-F][0-9a-fA-F])?$/;
var colorRegEx2 = /^rgb\(([^)]*)\)$/;
...
exports.parseColor = function parseColor(val) {
  ...
  // is it #aaa, #ababab or #abababaa
  if (res) {
    var defaultHex = val.substr(1);
    var hex = val.substr(1);
    if ([3, 4].indexOf(defaultHex.length) > -1) {
    ...
    if (hex.length === 8) {
      ...
      var hexAlphaToRgbaAlpha = (parseInt(hexAlpha, 16) / 255).toFixed(2)

      if (parseInt(hexAlphaToRgbaAlpha) == hexAlphaToRgbaAlpha) {
        hexAlphaToRgbaAlpha = parseInt(hexAlphaToRgbaAlpha)
      }
