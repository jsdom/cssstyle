...
const syntaxDefinitions = require("./generated/syntaxDefinitions");
...
/**
 * Retrieves a value from the cache or executes the provider function and caches the result.
 *
 * @param {string} key - The cache key.
 * @param {Function} provider - The function to execute if the key is not in the cache.
 * @returns {any} The cached or parsed value.
 */
function getCached(key, provider) {
...
function isValidPropertyValue(prop, val) {
  ...
  return getCached(`isValidPropertyValue_${prop}_${val}`, () => {
    try {
...
function isSyntaxCaseSensitive(syntax) {
  return getCached(`isSyntaxCaseSensitive_${syntax}`, () => {
    try {
      const ast = parseSyntax(syntax);
      if (ast.type === AST_TYPES.GROUP) {
        const types = new Set();
        csstree.definitionSyntax.walk(ast, {
          enter(node) {
            if (node.type === AST_TYPES.TYPE && !node.name.endsWith("()")) {
              types.add(node.name);
            }
          }
        });
        if (types.has("custom-ident") || types.has("dashed-ident") || types.has("string")) {
          return true;
        }
        for (const type of types) {
          const { syntax: typeSyntax } = syntaxDefinitions.get(`<${type}>`);
...
function resolveCalc(val, opt = { format: "specifiedValue" }) {
  ...
  return getCached(`resolveCalc_${val}`, () => {
