...
exports.parsePropertyValue = function parsePropertyValue(prop, val, globalObject) {
  val = exports.prepareValue(val, globalObject);
  if (val === "" || exports.hasVarFunc(val)) {
    return val;
  } else if (exports.hasCalcFunc(val)) {
    val = exports.parseCalc(val, {
      format: "specifiedValue"
    });
    if (exports.isValidPropertyValue(prop, val)) {
      return val;
    }
    return;
  } else if (GLOBAL_KEY.includes(asciiLowercase(val))) {
    return asciiLowercase(val);
  } else if (SYS_COLOR.includes(asciiLowercase(val))) {
    if (/^(?:-webkit-)?(?:[a-z][a-z\d]*-)*color$/i.test(prop)) {
      return asciiLowercase(val);
    }
    return;
