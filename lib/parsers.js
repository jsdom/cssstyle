...
exports.resolveCalc = function resolveCalc(val, opt = { format: "specifiedValue" }) {
  if (typeof val !== "string") {
    val = exports.prepareValue(val);
  }
...
// Parse <percentage>.
exports.parsePercentage = function parsePercentage(val, opt = {}) {
  const [item] = val;
  const { type, value } = item ?? {};
...
exports.parseLengthPercentage = function parseLengthPercentage(val, opt = {}) {
  const [item] = val;
  const { type, value, unit } = item ?? {};
