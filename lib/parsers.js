...
exports.resolveCalc = (val, opt = defaultOptions) => {
  if (typeof val !== "string") {
    val = exports.prepareValue(val);
  }
...
exports.parsePropertyValue = (prop, val, opt = {}) => {
  const { caseSensitive, globalObject, inArray, options } = opt;
  val = exports.prepareValue(val, globalObject);
  if (val === "" || exports.hasVarFunc(val)) {
    return val;
  } else if (exports.hasCalcFunc(val)) {
    const calculatedValue = exports.resolveCalc(val, options);
...
// Parse <color>.
exports.parseColor = (val, opt = defaultOptions) => {
  const [item] = val;
...
// Parse <gradient>.
exports.parseGradient = (val, opt = defaultOptions) => {
  const [item] = val;
  const { name, type, value } = item ?? {};
  if (type !== "Function") {
    return;
