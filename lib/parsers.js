...
exports.valueType = function valueType(val) {
  ...
  if (res !== null) {
  	parts = res[1].split(/[\s,\/]+/);
  	var rgb = parts.slice(0, 3).map(col => col.toLowerCase() == "none" ? 0 : col);
  	return (
  		(parts.length == 3 || parts.length == 4) && 
  		(rgb.every(percentRegEx.test.bind(percentRegEx)) || rgb.every(integerRegEx.test.bind(integerRegEx)))
  	) ? exports.TYPES.COLOR : undefined;
  }
...
exports.parseColor = function parseColor(val) {
  ...
  if (res) {
  	parts = res[1].split(/[\s,\/]+/);
  	var rgb = parts.slice(0, 3).map(col => col.toLowerCase() == "none" ? 0 : col);
  	if (rgb.every(percentRegEx.test.bind(percentRegEx))) {
      [red, green, blue] = rgb.map(code => Math.floor((parseFloat(code.slice(0, -1)) * 255) / 100));
  	} else if (rgb.every(integerRegEx.test.bind(integerRegEx))) {
  		[red, green, blue] = rgb.map(code => parseInt(code, 10));
  	} else {
  		return undefined;
  	}
  	[red, green, blue] = [red, green, blue].map(code => Math.min(255, Math.max(0, code)));
  	var base = [red, green, blue].join(', ');
  	if(parts.length == 4) {
  		alpha = parts[3] == "none" ? 0 : (percentRegEx.test(parts[3]) ? parseFloat(parts[3].slice(0, -1)) / 100 : parseFloat(parts[3]));
  		alpha = isNaN(alpha) ? 1 : Math.min(1, Math.max(0, alpha));
  	} else {
  		alpha = 1;
  	}
  	return (alpha >= 0 && alpha < 1) ? 'rgba(' + base + ', ' + alpha + ')' : 'rgb(' + base + ')';
  }
