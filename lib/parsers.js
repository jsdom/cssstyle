...
var lengthRegEx =
  /^(0|[-+]?[0-9]*\.?[0-9]+(in|cm|em|mm|pt|pc|px|ex|rem|vh|vw|ch))$/;
var percentRegEx = /^[-+]?[0-9]*\.?[0-9]+%$/;
var urlRegEx = /^url\(\s*([^)]*)\s*\)$/;
var stringRegEx = /^("[^"]*"|'[^']*')$/;
var colorRegEx1 = /^#([0-9a-fA-F]{3,4}){1,2}$/;
var colorRegEx2 =
  /^rgba?\(((?:(?:none|[-+]?[0-9]*\.?[0-9]+%?)[\s,/]*){2,3}(?:none|[-+]?[0-9]*\.?[0-9]+%?))\)$/;
var rgbdelimiters = /[\s,/]+/;
var calcRegEx = /^calc\(([^)]*)\)$/;
var colorRegEx4 =
...
exports.parseColor = function parseColor(val) {
  ...
  if (res) {
    ...
    if (hex.length === 8) {
      ...
      var hexAlphaToRgbaAlpha = Number(
        (parseInt(hexAlpha, 16) / 255).toFixed(3),
      );
  ...
  if (res) {
    ...
    if (parts.length == 4) {
      alpha =
        parts[3] == "none"
          ? 0
          : percentRegEx.test(parts[3])
            ? parseFloat(parts[3].slice(0, -1)) / 100
            : parseFloat(parts[3]);
    ...
    return (base.length == 4 ? "rgba(" : "rgb(") + base.join(", ") + ")";
  }
...
exports.subImplicitSetter = function (prefix, part, isValid, parser) {
  ...
  return function (v) {
    ...
    var priorities = subparts.map((subpart) =>
      this.getPropertyPriority(subpart),
    );
    ...
    if (
      ...
      for (var j = 0; j < subparts.length; j++) {
        ...
        var priority =
          subparts[j] === property ? "" : priorities[j] || combinedPriority;
