...
var colorRegEx2 = /^rgba?\(((?:(?:none|[-+]?[0-9]*\.?[0-9]+%?)[\s,\/]*){2,3}(?:none|[-+]?[0-9]*\.?[0-9]+%?))\)$/;
var rgbdelimiters = /[\s,\/]+/;
var calcRegEx = /^calc\(([^)]*)\)$/;
var colorRegEx4 =
  ...
  if (res) {
    ...
    [red, green, blue] = parts.slice(0, 3).map(code => 
      ...
      Math.min(255, Math.max(0, 
          percentRegEx.test(code) ? Math.round((parseFloat(code.slice(0, -1)) * 255) / 100) : 
          (integerRegEx.test(code) ? parseInt(code, 10) : undefined)
	)
      )
    );
