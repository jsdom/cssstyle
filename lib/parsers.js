...
exports.shorthandParser = function parse(v, shorthand_for) {
  ...
  const valid = parts.every((part, i) =>
    Object.keys(shorthand_for).some(property => {
      const value = shorthand_for[property].parse(part, i);
      if (value) {
        obj[property] = value;
        return true;
      }
...
exports.implicitSetter = function(property_before, property_after, parse) {
  ...
  return function(v) {
    ...
    if (parts.some((part, i) => (parts[i] = parse(part)) === undefined)) {
      return undefined;
    }
...
//
exports.subImplicitSetter = function(prefix, part, parse) {
  ...
  return function(v) {
    ...
    var parts = [];
    for (var i = 0; i < 4; i++) {
      if (this._values[subparts[i]] == null) {
        break;
      }
