 ...
 */
const isGlobalKeyword = (val) => GLOBAL_KEYS.includes(asciiLowercase(val));
...
const isValidPropertyValue = (prop, val, globalObject) => {
  ...
  if (SYS_COLORS.includes(asciiLowercase(val))) {
...
const processNumericValue = (item, allowedTypes, opt = {}) => {
  ...
  const isZero = type === AST_TYPES.NUMBER && parseFloat(value) === 0;
  const isValidType =
    allowedTypes.includes(type) ||
    (isZero &&
      (allowedTypes.includes(AST_TYPES.DIMENSION) || allowedTypes.includes(AST_TYPES.PERCENTAGE)));
  if (!isValidType) {
    return;
  }
  const { clamp } = opt;
...
const parsePropertyValue = (prop, val, opt = {}) => {
  ...
  if (GLOBAL_KEYS.includes(lowerCasedValue)) {
    if (inArray) {
      parsedValue = [
        {
          type: "GlobalKeyword",
  ...
  } else if (SYS_COLORS.includes(lowerCasedValue)) {
...
const parseNumber = (val, opt = {}) => {
  const [item] = val;
  const num = processNumericValue(item, [AST_TYPES.NUMBER], opt);
...
const parseLength = (val, opt = {}) => {
  const [item] = val;
  const num = processNumericValue(item, [AST_TYPES.DIMENSION], opt);
  if (typeof num !== "number") {
    return;
  }
  const { unit } = item ?? {};
...
const parsePercentage = (val, opt = {}) => {
  const [item] = val;
  const num = processNumericValue(item, [AST_TYPES.PERCENTAGE], opt);
  if (typeof num === "number") {
    return `${num}%`;
  }
};
...
const parseLengthPercentage = (val, opt = {}) => {
  const [item] = val;
  const num = processNumericValue(item, [AST_TYPES.DIMENSION, AST_TYPES.PERCENTAGE], opt);
  if (typeof num !== "number") {
    return;
  }
 ...
 */
const parseColor = (val, opt) => {
  const [item] = val;
  const { name, type, value } = item ?? {};
  switch (type) {
    case AST_TYPES.FUNCTION: {
      const res = resolveColor(`${name}(${value})`, opt ?? DEFAULT_OPTS);
      if (res) {
        return res;
      }
      break;
 ...
 */
const parseGradient = (val, opt) => {
  const [item] = val;
  const { name, type, value } = item ?? {};
  if (type !== AST_TYPES.FUNCTION) {
    return;
  }
  const res = resolveGradient(`${name}(${value})`, opt ?? DEFAULT_OPTS);
  if (res) {
    return res;
  }
};
