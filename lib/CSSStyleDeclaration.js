  ...
  _implicitSetter: {
    ...
    value(prefix, part, val, parser, positions = []) {
      ...
      if (val === "") {
        ...
        const value = parsePropertyValue(shorthandProp, val, {
          globalObject: this._global
        });
      ...
      if (!parts.length || parts.length > positions.length) {
        return;
      }
      const properties = parts.map((p) =>
        parser(p, {
          globalObject: this._global
        })
      );
      this._setProperty(shorthandProp, properties.join(" "));
      switch (positions.length) {
        case 4:
          if (properties.length === 1) {
            properties.push(properties[0], properties[0], properties[0]);
          } else if (properties.length === 2) {
            properties.push(properties[0], properties[1]);
          } else if (properties.length === 3) {
            properties.push(properties[1]);
          }
          break;
        case 2:
          if (properties.length === 1) {
      ...
      for (let i = 0; i < positions.length; i++) {
        ...
        this.removeProperty(property);
        this._values.set(property, properties[i]);
      }
    },
    enumerable: false
  },
