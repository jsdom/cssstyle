...
class CSSStyleDeclaration {
   ...
   * @param {"specifiedValue"|"computedValue"} [opt.format="specifiedValue"] - "specifiedValue" or "computedValue"
  ...
  *[Symbol.iterator]() {
   ...
   *
   * @private
   * @param {string|null} [originalText] - The CSS text before the change. If provided, checking compares this with new cssText.
   */
  ...
  _setProperty(property, val, priority) {
    ...
    // Only calculate cssText if we are notifying a change AND not currently in a bulk update.
    const originalText = (typeof this._onChange === "function" && !this._updating) ? this.cssText : null;
    
    // Performance Optimization:
    // Only scan array to add index if the property is not already known.
    if (!this._values.has(property)) {
...
for (const [methodName, handler] of Object.entries(internalSetters)) {
  // NOTE: Arrow functions cannot be used here because 'this' must be bound to the CSSStyleDeclaration instance.
