...
class CSSStyleDeclaration {
   ...
   * This setter is called by Array.prototype.splice.call().
   *
   * @param {number} len
   */
  set length(len) {
   ...
   * @param {string} [priority] - Priority (e.g. "important")
   */
  setProperty(prop, val, priority) {
    this._checkReadonly(prop);
    const value = prepareValue(val, this._global);
    if (value === "") {
      // Implicitly change longhand properties and remove property itself.
