...
var external_dependency_parsers_0 = require("./parsers.ts");

var external_dependency_constants_1 = require("./constants.ts");
...
azimuth_export_definition = {
  set: function (v) {
    ...
    if (valueType === external_dependency_parsers_0.TYPES.VAR) {
      return this._setProperty('azimuth', v);
    }
...
var background_local_var_shorthandSetter = external_dependency_parsers_0.shorthandSetter('background', background_local_var_shorthand_for);
var background_local_var_shorthandGetter = external_dependency_parsers_0.shorthandGetter('background', background_local_var_shorthand_for);
background_export_definition = {
  set: function (value) {
    const valueType = external_dependency_parsers_0.valueType(value);
    console.log('set background', value, valueType === external_dependency_parsers_0.TYPES.VAR);

    if (valueType === external_dependency_parsers_0.TYPES.VAR) {
      this._setProperty('background', value);

      Object.keys(background_local_var_shorthand_for).forEach(function (name) {
        const camel = external_dependency_parsers_0.dashedToCamelCase(name);
        this[camel] = external_dependency_parsers_0.PENDING_SUBSTIUTION_VALUE;
        this._values[name] = external_dependency_parsers_0.PENDING_SUBSTIUTION_VALUE;
      }, this);
    } else {
      return background_local_var_shorthandSetter.call(this);
    }
  },
  get: background_local_var_shorthandGetter,
  enumerable: true,
  configurable: true
};
